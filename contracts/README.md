...
| What | Where |
|------|-------|
| Proto contract (v3) | [quotes/v3/quotes_v3.proto](quotes/v3/quotes_v3.proto) — messages, service and `google.api.http` rules. Byte-for-byte the contract the sibling .NET service serves, apart from the `go_package` option and one load-bearing rpc reorder (`GetQuoteById` before `GetRandomQuote` — grpc-gateway pattern ties, see ADR 0002) |

...
These decisions were evaluated in parallel research passes before the stack was cut, and each
one pins the versions it depends on. Each of ADRs 0001-0009 records the context, the decision,
the alternatives rejected, the mapping onto the sibling .NET implementation of this contract and
the exact version pins; ADR 0010 amends ADR 0001 and carries neither a mapping nor pins of its
own. The
...
| --- | --- | --- | --- |
| [0001](adr/0001-orchestration-compose-traefik.md) | Orchestration: compose spec + Traefik v3 edge | compose file is dev topology and publish artifact; Traefik file-provider routes at the edge | traefik `v3.7.11`, postgres `18.6-alpine3.23`, pgweb `0.17.0`, nginx `1.31.4-alpine3.24`, node `24.19.0-alpine3.23` |
...
| [0004](adr/0004-configuration-viper.md) | Configuration: Viper with typed structs and fail-fast validation | one Viper instance, `__` env-name parity across the sibling implementations, boot-time validation | viper `v1.21.0` |
...
| [0006](adr/0006-auth-and-errors.md) | Auth, problem+json errors, rate limiting, routing, health | golang-jwt HS256 validated in-process; hand-rolled problem+json envelope and fixed-window limiter; stdlib mux | golang-jwt/jwt/v5 `v5.3.1` |
...
| [0009](adr/0009-lint-and-architecture-guard.md) | Lint and architecture guard: golangci-lint v2 + depguard | one tool for format and layering; depguard rules encode the layering table | golangci-lint `v2.13.1`, action `v9.3.0`, gofumpt `v0.11.0` |

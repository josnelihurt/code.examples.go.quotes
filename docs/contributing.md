...
Bumping the pins is a four-step, one-PR change (paired with the sibling
repository's own bump, re-freezing in the same change, so both frozen
documents stay generated by the same toolchain — see ADR 0003's addendum):
